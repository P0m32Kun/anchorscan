...
func TestBuildProjectDeliverableProjectsConfirmedAndNegativeByZone(t *testing.T) {
	...
	deliverable := BuildProjectDeliverable(project, zones, nil, verifications, time.Unix(100, 0))
	...
	// Severity takes precedence over the manual position.
	if deliverable.Zones[0].Confirmed[0].Title != "弱口令" || deliverable.Zones[0].Confirmed[1].Title != "过期组件" {
	...
	// Summary follows confirmed order: 弱口令 (high) then 过期组件 (medium).
	if deliverable.Summary[0].Number != 1 || deliverable.Summary[0].Title != "弱口令" {
		t.Fatalf("summary[0] = %#v", deliverable.Summary[0])
	}
	if deliverable.Summary[1].Number != 2 || deliverable.Summary[1].Title != "过期组件" || deliverable.Summary[1].Severity != "中危" {
...
func TestBuildProjectDeliverableEmptyWhenNoIncludedVerifications(t *testing.T) {
	...
	deliverable := BuildProjectDeliverable(ProjectMetadata{}, zones, nil, nil, time.Unix(1, 0))
...
func TestBuildProjectDeliverableMergesSummaryAcrossZonesByVulnerabilityKey(t *testing.T) {
	zones := []ProjectZone{{ZoneID: "I", Name: "I区"}, {ZoneID: "III", Name: "III区"}}
	verifications := []DeliverableVerification{
		{ID: "v1", VulnerabilityKey: "weak-password", ZoneID: "I", Outcome: "confirmed", Title: "弱口令", Severity: "high", Assets: []DeliverableAsset{{IP: "10.0.0.2", Port: 22, Display: "10.0.0.2:22"}}},
		{ID: "v2", VulnerabilityKey: "weak-password", ZoneID: "III", Outcome: "confirmed", Title: "弱口令", Severity: "high", Assets: []DeliverableAsset{{IP: "10.0.0.1", Port: 22, Display: "10.0.0.1:22"}, {IP: "10.0.0.2", Port: 22, Display: "10.0.0.2:22"}}},
	}

	deliverable := BuildProjectDeliverable(ProjectMetadata{}, zones, nil, verifications, time.Unix(1, 0))
	if len(deliverable.Summary) != 1 {
		t.Fatalf("summary rows = %d", len(deliverable.Summary))
	}
	if deliverable.Summary[0].Assets != "10.0.0.1:22, 10.0.0.2:22" {
		t.Fatalf("summary assets = %q", deliverable.Summary[0].Assets)
	}
	if deliverable.Stats != (DeliverableStats{Total: 1, High: 1}) {
		t.Fatalf("stats = %#v", deliverable.Stats)
	}
}
