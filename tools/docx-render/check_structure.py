...
def verify_template_slots(source: Path, template: Path) -> None:
    ...
    required = {
        ...
        "{{ network_zone.name }}",
        "{%p for ap in network_zone.access_points_text.splitlines() %}",
        "{{ ap }}",
        "{%p for ip in network_zone.tester_ips_text.splitlines() %}",
        "{{ ip }}",
        "{%p for target in network_zone.targets_text.splitlines() %}",
        "{{ target }}",
        "{%p if network_zone.exclusions_text %}",
        "{%p for exclusion in network_zone.exclusions_text.splitlines() %}",
        "{{ exclusion }}",
        "{%p if network_zone.notes_text %}",
        "{%p for note in network_zone.notes_text.splitlines() %}",
        "{{ note }}",
        "{%p for verification in network_zone.confirmed %}",
        "{{ verification.heading }}",
        "{%p for para in verification.description_lines %}",
        "{{ para }}",
