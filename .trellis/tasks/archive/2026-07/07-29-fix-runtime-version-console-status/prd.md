...
- [x] 已明确 release 与本地开发构建各自显示的版本来源，并有自动化验证证明显示值可追溯至实际构建输入。
- [x] 关键扫描阶段、进度与可行动错误仍可见；已列出的 banner/ANSI/冗余探测噪声不再干扰控制台，且相关自动化验证通过。
- [x] 可从指定 Run 的持久化事实解释 `completed_with_errors`；处置与事实一致，并有测试或可复现运维证据。
- [x] 每个子任务有独立的验收、验证和代码审查；父任务的交叉验收确认三项变更不会破坏 release-version 与扫描运行状态契约。

## Completion Reconciliation

- 版本契约由 PR #25 验证并记录；现有开发回退与 release linker 注入不需要产品代码修改。
- Console 规范化由 PR #9 和 #10 交付。
- `completed_with_errors` 根因处置由 PR #11、#12 和最终 SSH 路由修复 PR #23 交付。
- 三条工作流均已通过各自质量门；父任务没有额外产品代码交付。
