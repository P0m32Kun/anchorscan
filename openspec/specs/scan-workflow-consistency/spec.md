# scan-workflow-consistency Specification

## Purpose
TBD - created by archiving change fix-scan-workflow-consistency. Update Purpose after archive.
## Requirements
### Requirement: 端口输入格式收敛
系统 MUST 仅接受 `top1000`、单一数字端口范围（如 `100-1000`）或逗号分隔的 1-65535 数字端口作为扫描端口输入，并 MUST 拒绝 `top100`、`full`、`highrisk`、混合范围与列表、以及非法端口。

#### Scenario: rustscan 参数语义
- **WHEN** 用户提交 `top1000`
- **THEN** 系统使用 rustscan `--top`
- **WHEN** 用户提交 `100-1000`
- **THEN** 系统使用 rustscan `--range 100-1000`
- **WHEN** 用户提交 `80,443,8080`
- **THEN** 系统使用 rustscan `--ports 80,443,8080`
...
#### Scenario: 拒绝已移除格式
- **WHEN** 用户提交 `top100`、`full`、`highrisk` 或 `80,100-200`
