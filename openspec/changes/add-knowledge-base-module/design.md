...
列表展示名称、四级风险、CVE 和主要匹配标识；详情展示描述、通用检测命令和修复建议。命令保留 v1 占位符，目标替换属于检测命令 change。
