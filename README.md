...
确保本机已安装 `fathom`（必配：存活探测 + scan_target 内唯一端口/指纹引擎，未配置则无法扫描）、`nmap`（NSE 引擎）、`httpx`、`nuclei`，并在系统 `PATH` 中可找到。配置文件无需手动创建——首次运行会自动生成 `config/default.yaml`，工具路径从 PATH 自动检测。
...
| 写法 | 含义 |
|------|------|
| `top1000` | 常见 1000 端口（config/ports-top1000.txt 预设） |
| `100-1000` | 端口范围（fathom 自动折叠为范围写法） |
| `80,443,8080` | 端口列表（fathom 直接消费） |
...
单工具调用（独立功能，非扫描流水线；仅跑 nmap / httpx / nuclei 之一）：
