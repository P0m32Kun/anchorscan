...
The external lab lives at `$SHARED_LAB_DIR`, defaulting to `~/DEV/lab`, and uses `docker-compose.yml`. Set it once before starting or restarting the lab:
...
Pull through a mirror, then re-tag it back to `mariadb:11` so `$SHARED_LAB_DIR/docker-compose.yml` does not need to change:
...
Check if another scan is running. Only one active scan is allowed per database.
