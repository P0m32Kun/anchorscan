...
Expected: 服务监听 `127.0.0.1:19000`。在同一浏览器、1440x900 视口保存 `/`、`/tools/new`、一个现有 `/tools/<name>`、`/runs/<run-id>` 和 `/reports/<run-id>` 的截图到 `/tmp/anchorscan-web-baseline/`；如果测试库没有 run/report 数据，使用当前稳定开发库只读复制到 `/tmp` 后启动，不修改仓库数据。
...
在 `app.test.mjs` 增加 `runStatusSource = fs.readFileSync(new URL('./run-status.js', import.meta.url), 'utf8')`，并让时间、轮询和 stepper 测试在同一 `vm` context 中先执行 `source`、再执行 `runStatusSource`。在 `server_test.go` 将 run 页面断言扩展为同时包含且按序出现：
...
rtk git add internal/web/static/app.js internal/web/static/run-status.js internal/web/static/app.test.mjs internal/web/templates/run.html internal/web/server_test.go
...
- [ ] **Step 1: 添加叶子脚本加载与行为测试**

让 `app.test.mjs` 分别读取 `tool-form.js`、`report-ui.js`。工具上下文断言加载脚本后向 `[data-tool-form]` 注册一次 `submit`，报告上下文继续执行现有两组分布图断言，并按浏览器顺序先执行 `app.js` 后执行对应叶子脚本。`server_test.go` 断言：工具页为 `app.js` 后 `tool-form.js`，报告页为 `app.js` 后 `report-ui.js`。
...
```bash
rtk git add internal/web/static/app.js internal/web/static/app.test.mjs internal/web/static/tool-form.js internal/web/static/report-ui.js internal/web/templates/tool_page.html internal/web/templates/report.html internal/web/server_test.go
...
用 Task 1 同一数据库、浏览器和 1440x900 视口重拍相同 URL。Expected: 像素比较无预期外差异；run 状态刷新、工具提交/输出、报告分布/筛选/复制均可操作。出现差异时只回退最后一个叶子抽取，不添加覆盖 CSS 或兼容层。
