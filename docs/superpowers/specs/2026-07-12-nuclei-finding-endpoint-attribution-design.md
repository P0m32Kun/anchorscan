...
当前数据库中该条件精确匹配 5 条重复记录；HTML 只显示一条是因为 `report.Build` 在导出时去重。更新内容为 `ip=172.22.0.1`、`port=6379`、`protocol=tcp`。更新前后分别查询并保存结果；受影响行数必须恰好为 5，否则回滚并停止。本次不删除历史重复记录，避免把持久层去重扩张进端点归属修复。
...
- 数据库更新恰好影响 5 行，并保留可恢复备份；不删除重复记录。
...
- **错误历史更新**：使用完整条件、事务、备份和“恰好 5 行”断言，禁止按模板 ID 批量更新或顺手删除重复记录。
