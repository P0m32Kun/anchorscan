...
```bash
export SHARED_LAB_DIR="${SHARED_LAB_DIR:-$HOME/DEV/lab}"
docker compose -f "$SHARED_LAB_DIR/docker-compose.yml" up -d --build
```
...
```bash
TOMCAT_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-tomcat)
REDIS_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-redis)
MARIADB_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-mariadb)
SSH_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-ssh)
SMB_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-samba)
UNKNOWN_IP=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' lab-unknown)
```
...
| --- | --- | --- |
| Tomcat | `lab-tomcat` | `8080` |
| Redis | `lab-redis` | `6379` |
| MariaDB | `lab-mariadb` | `3306` |
| SSH | `lab-ssh` | `2222` |
| SMB | `lab-samba` | `445` |
| Unknown TCP | `lab-unknown` | `9099` |
...
Use `--ports 1-65535` only when you intentionally want every local listening service. Full local scans often include app IPC/proxy ports, and `nmap -sV` can be slow on those ports.
...
The lab pass is good enough when:
...
## Deployment And Preflight Checks
...
## Single Tool Runs Lab

An operator can run one engine outside the automated pipeline. Each run still writes to SQLite and produces a JSON report, so it shows up under the same `/runs/<run_id>` and `/reports/<run_id>` pages.
...
| Check | Expected |
| --- | --- |
| `anchorscan version` | Prints the current AnchorScan version |
| `anchorscan --version` | Same as above |
| Web Console footer | Shows the same version |
