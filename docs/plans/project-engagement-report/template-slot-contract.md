...
**状态：revised（2026-07-21）/ 已按用户逐项纠错，等待对新版可视占位符模板复核。**

本文只定义正式报告模板中哪里可变、如何变化、数据从哪里来。本文得到用户确认前：

- `tools/docx-render/out/project-report-placeholder-template.docx` 是供审阅的实验模板，不是正式运行时模板；
- 不继续制作或验收新的 DOCX 模板；
- 不据此修改数据库、Web UI 或正式导出接口；
...
> 本次测试共测试`{{ report.client_name }}``{{ conclusion.network_zone_names_text }}`所有设备，共发现漏洞`{{ conclusion.total }}`个，其中高危漏洞`{{ conclusion.high }}`个、中危漏洞`{{ conclusion.medium }}`个、低危漏洞`{{ conclusion.low }}`个。其中问题主要集中在`{{ conclusion.focus_text }}`这几个方面。
>
...
| `K-04` | 分级数量 | `T` | `conclusion.high/medium/low` | `D(V.severity)` | 严格按批准句式只输出高/中/低三个计数；critical 的正式展示口径须在实现前另行确认，不能在模板中擅加一句 |
...
8. **已修订——结论句式**：使用用户给定的两句文案与统计槽，不再使用 `has_confirmed`、`critical` 或人工 `conclusion.text` 槽。

新版实验模板已据此生成，等待用户逐页查看后再决定是否批准为正式模板基线。
