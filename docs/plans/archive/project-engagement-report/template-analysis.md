...
> **状态纠正（2026-07-22）：** 本文的旧“占位槽映射”和原型描述只作为源文档分析记录。正式逐位置槽位、占位符类型、数据来源和空值规则以已批准的 `template-slot-contract.md` 为准；运行时模板为 `tools/docx-render/templates/project-report.docx`。
