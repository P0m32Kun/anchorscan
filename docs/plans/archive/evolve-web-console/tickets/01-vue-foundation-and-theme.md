...
- [x] 前端类型检查、生产构建和 Go embed 接入 Makefile、PR CI 与 release。
- [x] 首次绘制前解析主题，显式偏好跨刷新保留并支持系统运行时变化。
- [x] 所有控制台表面、表单、浮层、表格、代码区和滚动条覆盖明暗令牌。
- [x] 侧边栏外观入口与配置页共享同一偏好来源。
- [x] 独立导出报告保持浅色打印行为。
- [x] 主题 smoke、键盘检查、1440px 明暗截图与 `make pr-check` 通过。
