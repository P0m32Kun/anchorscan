...
- [x] 常用筛选常驻，低频筛选进入“更多筛选”。
- [x] 每个候选保留一个主操作，命令、复制和资产级操作按上下文展开。
- [x] tabs、dialog、文件选择、粘贴与拖放具有完整键盘和焦点行为。
- [x] 普通错误与保存反馈替换阻塞式 `alert`；危险删除继续确认。
- [x] Vue 成为工作台交互状态唯一所有者，删除对应 `workbench.js` 逻辑。
- [x] 既有验证、证据和命令生成契约通过浏览器回归。
