...
- [x] 搜索常驻，低频筛选收纳，活动条件可见并可逐项移除。
- [x] 查询参数仍可分享、刷新和后退恢复。
- [x] 检测执行事实默认折叠，发现与资产结果保持阅读优先级。
- [x] popover、视图 tabs、分页、复制和命令 dialog 支持完整键盘路径。
- [x] 删除 `app.js` 中被替代的报告交互与内联状态操作。
- [x] Web 报告、HTML/DOCX 导出和批量命令行为回归通过。
