...
### Project
一次有明确交付目标的渗透测试任务，例如“甘肃电力内网安全检查任务”。Project 按 Network Zone 组织多个 Scan Runs，并最终产出一份 Project Report；它不代表一个固定目标范围或一套扫描参数。Project 的报告元数据包括任务名称、被测单位、报告标题、测试对象、任务说明、测试起止日期和测试人员，由项目管理界面维护。旧 Project 仍保留默认扫描目标/端口/排除项等字段以保证可读性，但新建/编辑 UI 不再采集这些字段。
...
| 术语 | 含义 | 载体 |
|------|------|------|
| ProjectZone | Project 内的 Network Zone，用于组织 Runs 与报告内容 | `store.ProjectZone` |
